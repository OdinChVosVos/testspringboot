...
con = psycopg2.connect(
        database="icsodugm", 
        user="icsodugm", 
        password="OiqFMwmegNjTH2NN-1Wh-DPUcQxNCQNk", 
...
if not(cur.fetchone()[0]):
        ...
        ("Номер пользователя" INT PRIMARY KEY NOT NULL,
        "Логин" TEXT NOT NULL UNIQUE,
        "Пароль" TEXT,
        "Достижения" INT NOT NULL,
        "Баллы" INT NOT NULL,
        "Статус доступа" INT NOT NULL);'''
        )
        con.commit()

cur.execute("SELECT * from bd ORDER by \"Номер пользователя\"")
rows = cur.fetchall()
...
print('id номер пользователя | логин | пароль | достижения | баллы | статус доступа\n', '\n'.join(rows))
...
# fin=open(Path(Path.cwd(), 'bd.txt', encoding='utf-8')
# database=fin.read()
# fin.close()
# if database=='':
#         fin=open(Path(Path.cwd(), 'bd.txt', 'w', encoding='utf-8')
#         fin.write('id номер пользователя|логин|пароль|достижения|баллы|статус доступа\n0|admin|admin|4|0|1')
#         fin.close()
...
def vxod():
        ...
        def rediting_bd():
                ...
                # f_in=open(Path(Path.cwd(), 'bd.txt', encoding='utf-8')
                # rdl=f_in.readlines()[1:]
                # f_in.close()
                        ...
                        if flag_admin:
                                try:       
                                        if (int(new_status.get())==1 or int(new_status.get())==0) and new_login.get()!='' and new_status.get()!='' and new_login.get() not in login_rdl:
                                                ...
                                                # f_in2=open(Path(Path.cwd(), 'bd.txt', encoding='utf-8')
                                                # konez=int(f_in2.readlines()[-1].split('|')[0])
                                                # f_in2.close()
                                                # f_in2=open(Path(Path.cwd(), 'bd.txt', 'a', encoding='utf-8')
                                                # f_in2.write(f'\n{konez+1}|{new_login.get()}|{new_parol.get()}|{0}|{0}|{new_status.get()}')
                                                # f_in2.close()
                                ...
                                if new_login.get() != '' and new_login.get() not in login_rdl:
                                        ...
                                        # f_in2.close()
                                        # f_in2=open(Path(Path.cwd(), 'bd.txt', 'a', encoding='utf-8')
                                        # f_in2.write(f'\n{konez+1}|{new_login.get()}|{new_parol.get()}|{0}|{0}|{0}')
                                        # f_in2.close()
                ...
                def udalenie():
                        ...
                        if int(nom_udalenie.get())>0:
                                ...
                                if (not(flag_admin) and rdl[int(nom_udalenie.get())].split(', ')[-1] != '1') or flag_admin:
                                        ...
                                        cur.execute("DELETE from bd WHERE \"Номер пользователя\" = %s;", (nom_udalenie.get()))
                                        con.commit()

                                        cur.execute("UPDATE bd SET \"Номер пользователя\" = \"Номер пользователя\"-1 WHERE \"Номер пользователя\" > %s;", (nom_udalenie.get()))
                                        con.commit()
                                        ...
                                        # f_in2=open(Path(Path.cwd(), 'bd.txt', 'w', encoding='utf-8')
                                        # f_in2.write(''.join(rdl_udalenie[:int(nom_udalenie.get())+1])+''.join(rdl_udalenie[int(nom_udalenie.get())+2:]))
                                        # f_in2.close()
                                        
                                        # f_in2=open(Path(Path.cwd(), 'bd.txt', encoding='utf-8')
                                        # rdl_udalenie=f_in2.readlines()
                                        # f_in2.close()
                                        # rdl_udalenie[-1]=rdl_udalenie[-1].replace('\n','')
                                        
                                        # f_in2=open(Path(Path.cwd(), 'bd.txt', 'w', encoding='utf-8')
                                        # f_in2.write(''.join(rdl_udalenie))
                                        # f_in2.close()
                                        
                                        for i in tree.get_children(): tree.delete(i)
                                        if flag_admin!=1:
                                                for i in range(len(rdl_udalenie)):
                ...
                def redach():
                        ...
                        if int(nom_redact.get())>0:
                                ...
                                if (not(flag_admin) and rdl[int(nom_redact.get())].split(', ')[-1] != '1') or flag_admin:
                                        ...
                                        cur.execute("UPDATE bd SET \"Логин\"= %s, \"Пароль\"= %s WHERE \"Номер пользователя\" = %s",
        ...
        if login.get() in login_rdl and parol.get() == rdl[login_rdl.index(login.get())].split(', ')[2]:
                ...
                text = open(Path(Path.cwd(), 'Data.txt'), 'a', encoding='utf-8')      
                ...
                root.resizable(False, False)
                pole=PhotoImage(file=Path(Path.cwd(), 'pole.png'))
                root.title('Angry gun')
                ...
                def start():
                    ...
                    fon=PhotoImage(file=Path(Path.cwd(), 'fon.png'))
                    root.config(bg='grey')
                    root.title('Angry gun')
                    root.state('zoomed')
                        ...
                        if flag!=1 and flagm!=1:
                            ...
                            def moving():
                                ...
                                if M-27<=xm[j]<=M+70 and 626-27<=ym[j]<=626+70:
                                   ...
                                   pygame.mixer.Channel(1).play(pygame.mixer.Sound(Path(Path.cwd(), 'Bah.mp3')))

                                   pygame.mixer.Channel(1).queue(pygame.mixer.Sound(Path(Path.cwd(), 'victory.mp3')))
                                  
                                   mibr_frames=[PhotoImage(file=Path(Path.cwd(), 'mibr.gif'), format='gif -index %i' %(i)) for i in range(4)]
                                   ...
                                   def change():
                                        ...
                                        if rdl_new[idd].split(', ')[3]=='0':
                                                ...
                                                # cur.execute("SELECT * from bd ORDER by \"Номер пользователя\"")
                                                # rdl_popadenia = cur.fetchall()
                                                # rdl_popadenia = [str(row)[1:-1].replace('\'', '') for row in rdl_popadenia]

                                                # new_zadacha=rdl_popadenia[idd].split(', ')
                                                # new_zadacha.pop(3)
                                                # new_zadacha.insert(3, '1')
                                                # rdl_popadenia[idd]=', '.join(new_zadacha)

                                                cur.execute("UPDATE bd SET \"Достижения\" = 1 WHERE \"Номер пользователя\" = %s", (str(idd)))
                                                con.commit()
                                                ...
                                                # fin=open(Path(Path.cwd(), 'bd.txt', encoding='utf-8')
                                                # rdl_popadenia=fin.readlines()
                                                # fin.close()

                                                # fin=open(Path(Path.cwd(), 'bd.txt', 'w', encoding='utf-8')
                                                # fin.write(''.join(rdl_popadenia))
                                                # fin.close()
                                                ...
                                                if h==70:
                                                        ...
                                                        cur.execute("UPDATE bd SET \"Достижения\" = 2 WHERE \"Номер пользователя\" = %s", (str(idd)))
                                                        con.commit()
                                                        ...
                                                        # fin=open(Path(Path.cwd(), 'bd.txt', 'w', encoding='utf-8')
                                                        # new_zadacha=rdl_popadenia[idd+1].split(', ')
                                                        # new_zadacha.pop(3)
                                                        # new_zadacha.insert(3, '2')
                                                        # rdl_popadenia[idd+1]=', '.join(new_zadacha)
                                                        # fin.write(''.join(rdl_popadenia))
                                                        # fin.close()
                                                ...
                                                if h==70 and v0==100:
                                                        ...
                                                        cur.execute("UPDATE bd SET \"Достижения\" = 3 WHERE \"Номер пользователя\" = %s", (str(idd)))
                                                        con.commit()
                                                ...
                                                if h==70 and alfa==45:
                                                        ...
                                                        cur.execute("UPDATE bd SET \"Достижения\" = 4 WHERE \"Номер пользователя\" = %s", (str(idd)))
                                                        con.commit()
                                ...
                                if ym[j]>=669:
                                    ...
                                    pygame.mixer.Channel(1).set_volume(1.0)
                                    pygame.mixer.Channel(1).play(pygame.mixer.Sound(Path(Path.cwd(), 'Baz.mp3')))
                            ...
                            pygame.mixer.Channel(1).set_volume(1.0)
                            pygame.mixer.Channel(1).play(pygame.mixer.Sound(Path(Path.cwd(), 'Bam.mp3')))
                    ...
                    gun45=PhotoImage(file=Path(Path.cwd(), 'gun45f.png'))
                    gun0=PhotoImage(file=Path(Path.cwd(), 'gun0f.png'))
                    gun90=PhotoImage(file=Path(Path.cwd(), 'gun90f.png'))
                    gun30=PhotoImage(file=Path(Path.cwd(), 'gun30f.png'))
                    gun60=PhotoImage(file=Path(Path.cwd(), 'gun60f.png'))

                    gun45s2=PhotoImage(file=Path(Path.cwd(), '45s2.png'))
                    gun0s2=PhotoImage(file=Path(Path.cwd(), '10s2.png'))
                    gun90s2=PhotoImage(file=Path(Path.cwd(), '85s2.png'))
                    gun30s2=PhotoImage(file=Path(Path.cwd(), '30s2.png'))
                    gun60s2=PhotoImage(file=Path(Path.cwd(), '60s2.png'))

                    gun45s3=PhotoImage(file=Path(Path.cwd(), '45s3.png'))
                    gun0s3=PhotoImage(file=Path(Path.cwd(), '10s3.png'))
                    gun90s3=PhotoImage(file=Path(Path.cwd(), '85s3.png'))
                    gun30s3=PhotoImage(file=Path(Path.cwd(), '30s3.png'))
                    gun60s3=PhotoImage(file=Path(Path.cwd(), '60s3.png'))

                    floor=PhotoImage(file=Path(Path.cwd(), 'floor.png'))
                    Mi=PhotoImage(file=Path(Path.cwd(), 'mi.png'))
                    pod=PhotoImage(file=Path(Path.cwd(), 'pod2.png'))
                    pod3=PhotoImage(file=Path(Path.cwd(), 'pod.png'))
                    scroll=PhotoImage(file=Path(Path.cwd(), 'scroll.png'))
                ...
                #     fin.close()
                    ...
                    pygame.mixer.Channel(0).play(pygame.mixer.Sound(Path(Path.cwd(), 'music.mp3')), loops=10)
                ...
                def achieves():
                    ...
                    torch_frames=[PhotoImage(file=Path(Path.cwd(), 'torch2.gif'), format='gif -index %i' %(i)) for i in range(5)]
                    ramka=LabelFrame(root, bg='grey0', bd=0)
                    ramka.place(x=200, y=100)
                    
                    troph=PhotoImage(file=Path(Path.cwd(), 'troph.png'))
                    trophopen=PhotoImage(file=Path(Path.cwd(), 'trophopen2.png'))
                ...
                #     fin.close()
                    ...
                    pygame.mixer.Channel(0).play(pygame.mixer.Sound(Path(Path.cwd(), 'castle.mp3')), loops=10)

                    pygame.mixer.Channel(1).set_volume(1.0)
                    pygame.mixer.Channel(1).play(pygame.mixer.Sound(Path(Path.cwd(), 'torch.mp3')), loops=10)
